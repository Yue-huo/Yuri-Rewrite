...
9. 阅读前序分片契约。如果当前义务依赖前序 node_id、obligation_id、thread_key 或计划状态，把前序摘要中实际出现的稳定标识写入 cross_shard_dependencies。不得引用当前分片新建的 node_id、obligation_id 或 thread_key，也不得自行改名或概括前序 thread_key。该字段必须是扁平字符串数组（例如 ["obligation:O-xxx", "thread:师徒"]），严禁输出对象；无依赖时返回空数组。
