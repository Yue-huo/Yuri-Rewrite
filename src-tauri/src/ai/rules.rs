...
R3_RESTORE_SOURCE：仅作为 R3_PRESERVE 的修复修饰符；当当前改写稿已经过度修改中性节点时，required_changes 可以且只能要求删除新增内容、恢复原文中性表达。它不是新的节点模式，也不得借修复名义添加变化。
...
mod tests {
    ...
    fn rule_pack_requires_classification_and_minimal_causality() {
        ...
        assert!(pack.contains("R3_RESTORE_SOURCE"));
    }
}
