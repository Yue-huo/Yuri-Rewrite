...
任务：严格执行当前分片改写契约。每个 obligation 都必须被正确处理并在最终正文中留下可引用证据，但 R3_PRESERVE 的合格证据是中性内容被保留，不是发生改写。
- R3_CAUSAL_TRANSFORM / R3_DERIVED_TRANSFORM 只做契约要求的最小充分重构；R3_SURFACE_ADAPT 只做原文明示的性别称谓、身体差异或场景相关外貌适配；R3_PRESERVE 仍统一执行全局姓名/代词映射，除此之外保留原句语义和中性词。
- 只在原场景正在观察身体/外貌、发生身体互动或外貌影响当下反应时，允许添加适量且符合设定的外貌描写。外貌可以写，但不能据此强加温柔、母性、柔弱、慈悲、爱美或其他人格。
- 禁止“身为女性/女人”“我一个女人”“同为女子”“枉为女性”等刻意自我标签；禁止把普通女性关系自动改成闺蜜、暧昧、依赖或母女关系。
- 偶像、榜样、英雄、巨人、巨兽、强者、造物主、学生、同伴、朋友、管理员、师父、前辈、对手、敌人、主人、孩子、家伙等中性词逐字保留；未列入映射的人物姓名不得删除、改名或改成代词。
- 修改尽量限制在影响节点原文证据所在句及必要相邻句；节点之外只做全局身份映射、语法联动和格式清理，禁止自由润色整章。
...
{{
  ...
  "protagonist_impact_nodes": [{{
    ...
    "gender_mechanisms": ["原文中可直接举证的性别机制；若仅有男性姓名/代词而事件本身中性，必须为空数组"],
...
5. 不要提出任何后续处理方向。
6. `protagonist_impact_nodes` 只记录原著事实，不规划后续处理。必须覆盖主角的行动、对话、心理、身体状态、决定、参与的全部互动、他人对主角的谈论/判断/回忆/误会/安排，以及主角不出场但此前行为造成后果的场景。真正无关的章节可以没有节点。节点“被收集”不代表后续必须修改。
...
12. `gender_mechanisms` 必须严格区分“主角在场”和“性别造成因果”：观察、记录、做决定、使用能力、担任造物主/学生/英雄等中性行为不构成性别机制；不能写“可受女性视角、动作表达或交流方式影响”来填充。只有原文明示的身体差异、性别称谓、恋爱/婚姻、性别化社会角色、性别偏见或身体接触边界才记录。拿不准时返回空数组。
13. `narrative_function`、`state_before`、`state_after` 和 `links` 必须严格限制在该 `chapter_index` marker 内已经实际发生的内容。章节末尾只出现事件开端时，不得把下一章才发生的对话、救援结果、赐予物品、关系建立或剧情推进提前写入本章节点；跨章延续应由下一章另建节点。
